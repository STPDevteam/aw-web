...
const config = {
  ...
  progressFilePath: './create-players-progress.json',
  
  // 并行处理设置
  concurrentLimit: 5,     // 同时处理的地址数量 (比登录操作更小，避免创建玩家请求冲突)
  batchSize: 20,          // 每批处理的地址数量
  delayBetweenBatches: 3000, // 批次之间的延迟（毫秒）
  
  // 单个请求设置
  delayBetweenAddresses: 500, // 地址之间的延迟
  
  // 玩家创建设置
  worldId: 'mn7av4kb76fb65eaygyzgxxcg97cqvfn', // 指定的世界ID
...
// 处理单个地址创建玩家
async function createPlayerForAddress(address, index) {
  try {
    // 使用createPlayerBatch函数创建玩家
    const createPlayerOperation = async () => {
      return await client.mutation('player:createPlayerBatch', { 
    ...
    // 执行创建玩家操作（带重试）
    const result = await retry(createPlayerOperation, config.maxRetries, config.retryDelay);
...
async function processAddressesInParallel(addresses, startIndex) {
  ...
  for (let i = 0; i < addresses.length; i++) {
    ...
    const processPromise = (async () => {
      console.log(`[${globalIndex + 1}/${totalAddresses}] 为地址创建玩家: ${address}`);
      const result = await createPlayerForAddress(address, globalIndex);
      
      if (result.success) {
        console.log(`  [${globalIndex + 1}/${totalAddresses}] ${address} - 成功创建玩家`);
        if (result.result.points) {
          console.log(`    当前积分: ${result.result.points}`);
        }
      } else {
        console.error(`  [${globalIndex + 1}/${totalAddresses}] ${address} - 创建失败: ${result.error}`);
      }
      
      // 每个地址处理后添加延迟，即使在并行情况下也是如此
